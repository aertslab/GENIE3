...
The algorithm outputs a matrix containing the weights of the putative regulatory links, with higher weights corresponding to more likely regulatory links. `weightMatr[i,j]` is the weight of the link directed from the $i$-th gene to $j$-th gene.
...
weightMatr <- GENIE3(exprMatr, regulators=regulators)
```

Here, only `Gene2`, `Gene4` and `Gene7` (respectively corresponding to rows 2, 4 and 7 in `exprMatr`) were used as candidate regulators. In the resulting `weightMatr`, the links that are directed from genes that are not candidate regulators have a weight equal to 0.
...
weightMatr <- GENIE3(exprMatr, treeMethod="ET", K=7, nTrees=50)
```
...
```{r eval=FALSE}
weightMatr <- GENIE3(exprMatr, nCores=4, verbose=TRUE)
```
...
```{r eval=FALSE}
linkList <- getLinkList(weightMatr, reportMax=5)
```
...
```{r eval=FALSE}
linkList <- getLinkList(weightMatr, threshold=0.1)
```
